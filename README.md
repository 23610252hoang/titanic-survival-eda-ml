...
[![Portfolio validation](https://github.com/23610252hoang/titanic-survival-eda-ml/actions/workflows/portfolio-validation.yml/badge.svg)](https://github.com/23610252hoang/titanic-survival-eda-ml/actions/workflows/portfolio-validation.yml)
...
├── data/
│   ├── README.md
│   └── sample_titanic.csv  # 公開用の完全な合成サンプル
├── notebooks/
...
元データは授業・課題用のデータであるため、公開リポジトリには含めていません。代わりに、列構成と欠損値処理を確認できる完全な合成データ `data/sample_titanic.csv` を公開しています。このサンプルは実在人物・授業データを含まず、学習済みAccuracyの再現用ではありません。

元実験をローカルで再現する場合は、許可されたCSVを以下に配置します。
...
公開サンプルのschemaとNotebook構造は `scripts/validate_portfolio.py` で検証できます。元実験の完全実行とAccuracy 0.873の再現には、README記載の許可済みCSVを `data/raw/` へ配置する必要があります。
...
本リポジトリで作成したソースコードとドキュメントは [MIT License](LICENSE) です。授業データを含む第三者データはMIT Licenseの対象外で、それぞれの利用条件に従います。
